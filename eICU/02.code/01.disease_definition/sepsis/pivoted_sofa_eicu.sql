...
drop table if exists pivoted_sofa_eicu;
create table pivoted_sofa_eicu as
...
(
  select patientunitstayid
  , GENERATE_ARRAY(-24, CAST(ceil(los_icu_hours) AS INT)) as hr
  from cohort_info
)
...
(
  ...
  from pivoted_blood_pressure_eicu pv
...
(
  ...
  from pafi_0 pf
  left join pivoted_vent_eicu pv
...
(
  ...
  left join norepinephrine_info_eicu ni 
  ...
  left join epinephrine_info_eicu ei 
  ...
  left join dobutamine_info_eicu di 
  ...
  left join dopamine_info_eicu dop 
